...
const EstudiosList = ({ infoCursos, infoMenu }) => {
  const [categoriaSeleccionada, setCategoriaSeleccionada] = useState("PROGRAMACION");
  ...
  const proyectosFiltrados =
    categoriaSeleccionada === "PROGRAMACION"
      ...
      : infoCursos.filter(
          (proyecto) =>
            proyecto.categoria.toLowerCase() === categoriaSeleccionada.toLowerCase()
        );
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (window.scrollY > estudiosSection.offsetTop + offset) { // Si el usuario ha desplazado lo suficiente hacia abajo
    ...
    return () => { // Retorna una función de limpieza que se ejecutará al desmontar el componente
          ...
          {proyecto.id ? (
            proyecto.id % 2 !== 0 ? (
              <div className="estudios-timeLine">
                ...
                <div data-aos="fade-up" className="estudios-timeLine_line">
                  <TbPointFilled />
                </div>
